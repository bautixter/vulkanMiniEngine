...
SsaoPassVK::SsaoPassVK(
    ...
    m_out_ssao_attachment(i_out_ssao_attachment)
{
...
void SsaoPassVK::initAuxStructures() {
    RendererVK& renderer = *m_runtime.m_renderer;
    VkDevice device = renderer.getDevice()->getLogicalDevice();

    // Generar 64 muestras en hemisferio
    ...
    for (uint32_t i = 0; i < SSAO_KERNEL_SIZE; ++i) {
        Vector3f sample(
            randomFloats(generator) * 2.0f - 1.0f,  // Entre [-1, 1]
            randomFloats(generator) * 2.0f - 1.0f,
            randomFloats(generator)                // Entre [0, 1] (hemisferio)
        );

        // Normalizar y escalar para distribuir ms cerca del origen
        sample = glm::normalize(sample);
        sample *= randomFloats(generator); // Escala aleatoria

        // Distribucin ms densa cerca del origen (lerp cuadrtico)
        float scale = static_cast<float>(i) / SSAO_KERNEL_SIZE;
        scale = glm::mix(0.1f, 1.0f, scale * scale);
        sample *= scale;

        m_ssao_kernel[i] = Vector4f(sample, 0.0f); // Almacenar como vec4
    }

    // Crear buffer de Vulkan para el kernel
    VkDeviceSize bufferSize = sizeof(Vector4f) * SSAO_KERNEL_SIZE;
    UtilsVK::createBuffer(*renderer.getDevice(), bufferSize,
        VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        m_kernel_buffer.buffer,
        m_kernel_buffer.memory
    );

    // Mapear y copiar datos
    void* data;
    vkMapMemory(device, m_kernel_buffer.memory, 0, bufferSize, 0, &data);
    memcpy(data, m_ssao_kernel.data(), bufferSize);
    vkUnmapMemory(device, m_kernel_buffer.memory);

    // Crear textura de ruido //
    // Generamos datos de ruido
    std::array<glm::vec2, 16> ssao_noise;
...
VkCommandBuffer SsaoPassVK::draw(const Frame& i_frame) {
    RendererVK& renderer = *m_runtime.m_renderer;
    VkDevice device = renderer.getDevice()->getLogicalDevice();
    uint32_t currentImageId = renderer.getWindow().getCurrentImageId();

    // Check if command buffer exists
    if (currentImageId >= m_command_buffer.size()) {
        throw std::runtime_error("Invalid command buffer index!");
    }

    VkCommandBuffer& current_cmd = m_command_buffer[currentImageId];    // Reset command buffer (with proper error handling)
    if (current_cmd != VK_NULL_HANDLE) {
        VkResult resetResult = vkResetCommandBuffer(current_cmd, 0);
    ...
    if (vkBeginCommandBuffer(current_cmd, &beginInfo) != VK_SUCCESS) {
...
void SsaoPassVK::createFbo()
{
    ...
    for (size_t i = 0; i < m_fbos.size(); i++)
    {
        ...
        attachments[0] = m_out_ssao_attachment.m_image_view; // Color attachment is the view of the swapchain image
...
void SsaoPassVK::createRenderPass()
{
    ...
    std::array<VkAttachmentDescription, 1> attachments = {};    // Ssao output attachment
    attachments[0].format = m_out_ssao_attachment.m_format;
...
void SsaoPassVK::createDescriptors()
{
    ...
    {
        { VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER        , 15 },
    ...
    for (uint32_t i = 0; i < m_runtime.m_renderer->getWindow().getImageCount(); i++)
    {
        ...
        binfo.offset = 0;
        binfo.range = sizeof(PerFrameData);        VkDescriptorBufferInfo kernelInfo;
        kernelInfo.buffer = m_kernel_buffer.buffer;
        kernelInfo.offset = 0;
        kernelInfo.range = sizeof(Vector4f) * SSAO_KERNEL_SIZE;
        ...
        image_infos[2].sampler = m_noise_texture.m_sampler;
        image_infos[2].imageView = m_noise_texture.m_image_view;
